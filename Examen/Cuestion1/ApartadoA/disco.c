...
int tres_clientes = 0;
int clean_pending = 1; //1: dentro de enter_cleaning()
...
void enter_normal_client(int id)
{
	...
	while(total_disco == CAPACITY || total_vip_queue > 0 || turno != turno_normal){
		total_normal_queue++;
...
void enter_vip_client(int id)
{
	...
	while(total_disco == CAPACITY || turno != turno_vip){
		total_vip_queue++;
...
void disco_exit(int id, int isvip)
{
	...
	if(isvip == 0) //cliente normal
		printf("Sale de la discoteca el cliente normal %d\n", id);
	else //cliente vip
		printf("Sale de la discoteca el cliente vip %d\n", id);

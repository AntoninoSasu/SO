...
pid_t launch_shell_command(const char* command){
    char path[10][MAX_LINE];
    strcpy(path[0], RUTA_BASH);
    strcpy(path[1], "-c");
    strcpy(path[2], command);
    //snprintf(path, sizeof(path), "%s -c %s", RUTA_BASH, command);

    pid_t pid = fork();
    if(pid == 0){ //child
        execl(RUTA_BASH, path[0], path[1], path[2], NULL);
        perror("execL");
        exit(EXIT_FAILURE);
    }
    else if(pid == -1){
...
int main(int argc, char *argv[]) {
    ...
    int opt;
    bool opcionB = false, opcionBMayus = false, opcionI = false;
    char* archivo;
    ...
    if(opcionBMayus == true && opcionI == true){
        char command[1024];
        while(1){
            printf("> ");
            fflush(stdout);

...
pthread_cond_t cond_dancefloor1;
pthread_cond_t cond_dancefloor2;

void enter_normal_client(int id, int dancefloor)
{
    ...
    {
        printf("Client %2d (not vip) waiting to enter assigned to dancefloor %d\n", id, dancefloor);
    ...
    printf("Client %2d (not vip) entered the disco to dancefloor %d (inside: %d)\n", id, dancefloor, inside);

    pthread_mutex_unlock(&mutex); //desbloquea
}

void enter_vip_client(int id, int dancefloor)
{
    ...
    {
        printf("Client %2d (  vip  ) waiting to enter to dancefloor %d (full capacity)\n", id, dancefloor);
        pthread_cond_wait(&cond_vip, &mutex);
    }

    waiting_vip--;  // Decrementa el número de VIPs esperando
    inside++;       // Incrementa el número de clientes dentro
    printf("Client %2d (  vip  ) entered the disco to dancefloor %d (inside: %d)\n", id, dancefloor, inside);

    pthread_mutex_unlock(&mutex);  //desbloquea
}

void enter_dancefloor1(int id, int is_vip, int dancefloor){
    pthread_mutex_lock(&mutex);

    waiting_d1++;   

    while(d1 >= CAPACITY_DANCEFLOOR){
        printf("Client %2d waiting to enter to dancefloor 1 (full capacity)\n", id);
        pthread_cond_wait(&cond_dancefloor1, &mutex);
    }
...
void enter_dancefloor2(int id, int is_vip, int dancefloor){
    pthread_mutex_lock(&mutex);

    waiting_d2++;   

    while(d2 >= CAPACITY_DANCEFLOOR){
        printf("Client %2d waiting to enter to dancefloor 2 (full capacity)\n", id);
        pthread_cond_wait(&cond_dancefloor2, &mutex);
    }
...
void *client(void *arg)
{
    ...
    if (isvip)
        enter_vip_client(id, dancefloor);
    else
        enter_normal_client(id, dancefloor);
...
int main(int argc, char *argv[])
{
    ...
    pthread_t threads[M];
    int args[M][3];  // Almacena id y isvip para cada hilo //Matriz para almacenar los argumentos (id y isvip) de cada hilo
    ...
    for (int i = 0; i < M; i++)
    {
        int isvip_char, dancefloor;
        fscanf(file, "%d %d", &isvip_char, &dancefloor);
